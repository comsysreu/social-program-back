...
export class AuthService {
    ...
    async validateUser(username: string, pass: string): Promise<any> {
        ...
        const user = await this.generic.findByQuery(this.entity, [{ $match: { $and: [{ statusD: 1 }, { username }] } }]);
        ...
        if (user.length > 0) {
            ...
            if (user[0].status === 0)
                throw new HttpException('Usuario bloqueado temporalmente', HttpStatus.BAD_REQUEST);
    ...
    async login(user: any) {
        const payload = { username: user.username, _id: user._id, profileId: user.profileId };
        return {
